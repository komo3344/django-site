...
class TestView(TestCase):
    ...
    def test_comment_form(self):
        ...
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn('Log in leave a comment', comment_area.text)
        ...
        response = self.client.post(
            ...
            {
                'content': 'user1의 댓글입니다.',
            },
            # POST로 보내는 경우 서버에서 처리한 후 리다리엑트 되는데 이때 따라가도록 함
            follow=True
        )
        ...
        self.assertIn('user1', new_comment_div.text)
        self.assertIn('user1의 댓글입니다.', new_comment_div.text)

    def test_comment_update(self):
        comment_by_user2 = Comment.objects.create(
            post=self.post_001, author=self.user2,
            content='user2의 댓글입니다.'
        )
        response = self.client.get(self.post_001.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        soup = BeautifulSoup(response.content, 'html.parser')
        ...
        self.assertIn('user1의 댓글을 수정합니다.', comment_001_div.text)
    ...
    def test_delete_comment(self):
        ...
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn('user2의 댓글입니다.', comment_area.text)
    ...
    def test_search(self):
        post_about_python = Post.objects.create(
            ...
            content ='Hello World. We ar the world.',
            author=self.user2
        )
